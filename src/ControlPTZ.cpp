...
ControlPTZ::ControlPTZ()
{
	...
	curl_easy_setopt(handle, CURLOPT_SSL_VERIFYHOST, 0L);
  	...
  	//curl_easy_setopt(handle, CURLOPT_USERPWD, "opera:clm2pO"); //for https
...
void ControlPTZ::HTTPRequestPTZ(int pan, int tilt, int zoom)
{
	...
	oss << "http://axis-ptz2/axis-cgi/com/ptz.cgi?continuouspantiltmove=";
	oss << pan;
	oss << ",";
	oss << tilt;
	oss << "&rzoom=" << zoom;
...
void ControlPTZ::HTTPRequestPTZPosRelative(float pan, float tilt, float zoom)
{
	...
	oss << "http://axis-ptz2/axis-cgi/com/ptz.cgi?rpan=" << pan;
	oss << "&rtilt=" << tilt;
	oss << "&rzoom=" << zoom;
...
void ControlPTZ::HTTPRequestPTZPosAbsolute(float pan, float tilt, float zoom)
{
	...
	oss << "http://axis-ptz2/axis-cgi/com/ptz.cgi?pan=" << pan;
	oss << "&tilt=" << tilt;
	oss << "&zoom=" << zoom;
